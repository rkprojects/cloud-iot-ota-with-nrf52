...
int gprs_ssl_recv(int session_id, unsigned char* buf, int buf_len, int timeout_ms)
{
	...
	if (ret < 0)
	{
		if (ret == GPRS_ERROR_TIMEOUT) return GPRS_ERROR_TIMEOUT;
		else return GPRS_ERROR_RECV_FAILED;
	}
...
int gprs_ssl_cert_download(const char* ro_fs_path)
{
	...
	if (ret < 0) return GPRS_ERROR_CERT_READ_FAILED;
...
int gprs_ssl_cert_delete(const char* ro_fs_path)
{
	...
	if (ret < 0) return ret;
	if (ret == 0) return GPRS_ERROR_NO_SUCH_CERTIFICATE;
